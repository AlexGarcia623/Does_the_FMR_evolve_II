...
for index, ax in enumerate(axs):
    ...
    colors = make_SFMS_fig(sim, ax)
    
    ax.text( 0.05, 0.825, WHICH_SIM_TEX[sim],
             transform=ax.transAxes )

ymin, ymax = axs[0].get_ylim()
axs[0].set_ylim(-(abs(ymin)*1.25), ymax*1.03)
...
axs[0].text(-0.175,0,r'$\log ({\rm SFR}~[M_\odot/{\rm yr}])$',
...
plt.savefig(savedir + 'SFMS_Comp.pdf', bbox_inches='tight')